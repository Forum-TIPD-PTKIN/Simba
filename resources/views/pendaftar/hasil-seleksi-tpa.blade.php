...
@if ($is_pengumuman_seleksi_tpa && in_array($status_seleksi_tpa?->status, ['GAGAL TPA', 'LOLOS TPA']))
    @if ($status_seleksi_tpa?->status === 'LOLOS TPA')
        <div class="alert alert-success">
            ...
            <p class="mb-0">Anda dinyatakan <span class="fw-bold fs-5">LOLOS NOMINASI SURVEYOR</span> Beasiswa
    ...
    @else
        <div class="alert alert-danger">
            ...
            <p class="mb-0">Anda
                dinyatakan <span class="fw-bold fs-5">TIDAK LOLOS NOMINASI SURVEYOR</span> Beasiswa
                {{ $pendaftar->beasiswa?->nama }} Tahun {{ $pendaftar->tahun_kegiatan?->tahun }}.
            </p>
        </div>
    @endif
