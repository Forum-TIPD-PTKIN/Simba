@if (!isset($nowizard) || $nowizard !== true)
    <h5 class="mb-3">Step 2: Biodata</h5>
@endif
<div class="row">
    <div class="col-12">
        @foreach ($generated_form as $item)
            <div class="card">
                ...
                <div class="card-body">
                    <form enctype="multipart/form-data" action="{{ route('pendaftar.biodata.store') }}" id="form-biodata"
                        method="POST">
                        @csrf
                    ...
                    <div class="d-flex justify-content-end">
                        <button onclick="simpanBiodata()" class="btn btn-success">Simpan</button>
                    </div>
                </div>
            </div>
        @endforeach
...
@push('script')
    <script>
        function viewControl(e) {
            let container = document.getElementById('form-biodata');
            let links = container.querySelectorAll('.base-biodata');
            let urls = []
            links.forEach(link => {
        ...
        function simpanBiodata() {
            Swal.fire({
                ...
                didOpen: () => {
                    Swal.showLoading();
                    let form = document.getElementById('form-biodata');
