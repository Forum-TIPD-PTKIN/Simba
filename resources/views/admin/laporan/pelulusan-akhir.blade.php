...
@section('content')
    ...
    <div class="pc-container">
        <div class="pc-content">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-12">
                            <ul class="breadcrumb">
                                ...
                                <li class="breadcrumb-item" aria-current="page">Pelulusan Akhir</li>
                            </ul>
                        </div>
                        <div class="col-md-12">
                            <div class="page-header-title">
                                <h2 class="mb-0">Pelulusan Akhir</h2>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="btn-group mb-3" role="group" aria-label="Button Impor Pelulusan Akhir">
                                <button type="button" class="btn btn-sm btn-dark" data-bs-toggle="modal"
                                    data-bs-target="#modalImporPelulusanAkhir"><span class="ti ti-file-import"></span>
                                    Impor Pelulusan Peserta Survei</button>
                            </div>
            ...
            <!-- Modal -->
            <div class="modal fade" id="modalImporPelulusanAkhir" tabindex="-1"
                aria-labelledby="modalImporPelulusanAkhirLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <form class="needs-validation" enctype="multipart/form-data" id="formImporPelulusanAkhir">
                        @csrf
                        <div class="modal-content">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="modalImporPelulusanAkhirLabel">Impor Pelulusan Peserta
                                    Survei
                                </h1>
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <small id="file_imporHelp" class="form-text text-muted">Unduh file template <a
                                            ...
                                            id="fileTemplateImporPelulusanAkhir">disini!</a></small>
                                </div>
                                <div class="mb-3">
...
@push('script')
    ...
    <script>
        ...
        const dataTable = $("#dataTable").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('admin.seleksi-akhir.pelulusan.data') }}",
                type: "POST",
                data: (data) => {
    ...
    <script>
        ...
        const validForm = $("form.needs-validation").validate({
            ...
            submitHandler: function(form, e) {
                ...
                // Serialize form
                const formData = new FormData($('#formImporPelulusanAkhir')[0]);
                formData.append('_token', "{{ csrf_token() }}");

                $.ajax({
                    type: "POST",
                    url: "{{ route('admin.seleksi-akhir.pelulusan.impor') }}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    cache: false,
                    success: function(data) {
                        form.reset();
                        $('#modalImporPelulusanAkhir').modal('hide');
                        dataTable.ajax.reload(null, false);
                    ...
                    error: function(error) {
                        ...
                        Swal.fire({
                            ...
                            text: error && error.status !== 200 ?
                                ...
                                'Gagal impor pelulusan peserta survei',
    ...
    <script>
        // Fungsi ubah status
        function ubahStatusKelulusanAkhir(status_id, status) {
            let url = "{{ route('admin.seleksi-akhir.pelulusan.update') }}";

            Swal.fire({
                title: 'Anda Yakin?',
                html: `Anda akan mengubah status kelulusan peserta survei menjadi <strong>${status}</strong>`,
                icon: 'question',
        ...
        function lolosData(id) {
            ...
            ubahStatusKelulusanAkhir(status_id, status);
        }

        // Tidak Lolos
        function gagalData(id) {
            const status_id = id;
            const status = 'Tidak Lolos';
    ...
    <script>
        $(document).on('click', '#fileTemplateImporPelulusanAkhir', function() {
            const tahun = $('#tahun_kegiatan').val(),
                beasiswa = $('#beasiswa').val();

            $.ajax({
                url: "{{ route('admin.seleksi-akhir.pelulusan.template') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    tahun: tahun,
                ...
                success: function(response, status, xhr) {
                    ...
                    var filename = (matches != null && matches[1] ? matches[1] :
                        'template_impor_pelulusan_peserta_survei.xlsx');
