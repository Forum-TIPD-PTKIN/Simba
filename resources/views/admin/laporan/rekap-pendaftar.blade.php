                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="btn-group mb-3" role="group" aria-label="Button Generate Peserta CBT">
                                <button type="button" class="btn btn-sm btn-success" id="unduhDataPendaftar"><span
...
@push('script')
    ...
    <script>
        $(document).on('click', '#unduhDataPendaftar', function() {
            const tahun = $('#flt_tahun').val(),
                beasiswa = $('#flt_beasiswa').val(),
                status = $('#flt_status').val();

            $.ajax({
                url: "{{ route('admin.laporan.rekap.unduh') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    tahun: tahun,
