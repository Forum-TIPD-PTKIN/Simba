    ...
    <div class="pc-container" id="app">
        <div class="pc-content">
            ...
            <div class="card">
                <div class="card-body">
                    <div class="row mb-3">
                        <label for="flt_tahun" class="col-sm-3 col-form-label">Tahun Kegiatan</label>
                        <div class="col-sm-9">
                            <select class="form-select" id="flt_tahun" aria-label="Default select example">
                                <option value="" selected disabled> - Pilih Tahun Kegiatan - </option>
                                @foreach ($list_tahun as $item)
                                    <option value="{{ $item->id }}" @selected($item->id == $tahun)>
                                        {{ $item->tahun }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="row mb-3">
                        <label for="flt_beasiswa" class="col-sm-3 col-form-label">Beasiswa</label>
                        <div class="col-sm-9">
                            <select class="form-select" id="flt_beasiswa" aria-label="Default select example">
                                <option value="" selected disabled> - Pilih Beasiswa - </option>
                                @foreach ($list_beasiswa as $item)
                                    <option value="{{ $item->id }}" @selected($item->id == $beasiswa)>
                                        {{ $item->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="row mb-3">
                        <label for="flt_peserta" class="col-sm-3 col-form-label">Peserta</label>
                        <div class="col-sm-9">
                            <select class="form-select select2" id="flt_peserta" aria-label="Default select example">
                    ...
                    <div class="row justify-content-end">
                        <button type="button" class="btn btn-primary" onclick="reloadPage()">Pilih</button>
                    </div>
                </div>
            </div>
...
@push('script')
    ...
    <script>
        ...
        function saveOnce(key, data, el, sesuai) {
            ...
            $.ajax({
                url: "{{ route('admin.seleksi-akhir.survei.post') }}",
                type: "POST",
                data: {
                    _token: '{{ csrf_token() }}',
                    key,
...
@push('script')
    ...
    <script>
        ...
        function reloadPage() {
            const id = $('#flt_peserta').val(),
                tahun = $('#flt_tahun').val(),
                beasiswa = $('#flt_beasiswa').val();
            let url =
                `{!! route('admin.seleksi-akhir.survei', ['id' => 'ID', 'tahun' => 'TAHUN', 'beasiswa' => 'BEASISWA']) !!}`;
        ...
        $('#flt_tahun, #flt_beasiswa').on('change', function() {
            $.ajax({
                url: "{{ route('admin.seleksi-akhir.survei.peserta') }}",
                type: 'GET',
                data: {
